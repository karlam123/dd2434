...
def nstart(dics, K):
    ...
    for m, doc in dics.iteritems():
        nmk[m] = np.zeros(K)
        for v, word in doc.iteritems():
            if v not in nvk:
                nvk[v] = K*[0]
            nvk[v][word[1]] += word[0]
            nk[word[1]] += word[0]
...
def common_words(dics, nvk, nk, sigma, K):
    ...
    words = [[0]*N]*K
    for k in range(K):
        words[k] = map(lambda i:DIC[i], map(int, indices[k, :]))
    common = np.zeros((K, N))
    for k in range(K):
        common[k, :] = B[k, map(int,indices[k, :])]
...
def handle_results():
    ...
    def fileP(file, K):
        alpha = 0.3
        sigma = 0.3
        return file+"_K="+str(K)+",alpha="+str(alpha)+",sigma="+str(sigma)+",iterations=500.p"
    for K in Ks:
        file1 = fileP("dics", K)
        file2 = fileP("nvk", K)
        file3 = fileP("nmk", K)
        ...
        thetam = doc_distr(dics, nmk, nm, 0.3, K, 10)
        print(words)
handle_results()
